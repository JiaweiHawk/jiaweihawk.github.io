...
static int64_t buddy_init(void) {
	...
	while(chunk < heapEnd) {
		...
		//首先设置内存的映射关系
		ptr_page2idx[(chunk - heapStart) >> LOG_PAGE_SIZE] = buddy_size - 1;
...
#### 获取内存
  ...
  最后，其从对应的表头数组下标处获取一个内存对象并返回即可;如果当前表头数组下标处没有可用的内存对象，则二分表头数组中最近的更大的内存对象，然后返回即可;如果仍然没有符合条件的，则返回NULL即可
  ```c
/*
	...
	if(bin->bk != bin) {
		...
		if(split) {
			...
			while(splitIndex-- != idx) {
				...
				buddy->ptr_page2idx[((uintptr_t)remainder - buddy->startAddress) >> LOG_PAGE_SIZE] = splitIndex;
			}

			buddy->ptr_page2idx[((uintptr_t)split - buddy->startAddress) >> LOG_PAGE_SIZE] = idx;
			unlock(&(buddy->lock_buddy));
			return split;
		}
	}
