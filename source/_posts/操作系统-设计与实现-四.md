...
#### 初始化伙伴算法
  由于伙伴算法涉及到对象的拆分和合并，其往往需要互斥的访问一些共享资源(比如位图)。因此在本实验中，其属于实验指南中的"slow path"，用来分配大内存或者当用于分配中、小内存的**slab机制**耗尽资源时的内存分配
  前面的实验要求中，需要返回的地址是$2^{i}$对齐的，因此这里将**Buddy**算法的每一个大小的链表块数都为上一个大小的链表块数的一半;并且**Buddy**结构中最后一个大小的内存对象的最后一块内存的结束地址位于**heap.end**，这样即可以保证最后返回的内存空间是$2^{i}$对齐的。对于**Buddy**算法的具体内存格式，由于系统最多分配**16MB**的内存对象(实验要求中说明)，因此我们保留**16MB**的内存空间，在地址对齐的前提下将其作为堆栈管理的初始化空间即可。剩下的内存空间从**Buddy**最后一个链表开始进行，按照前面的规则进行分配即可
  ```
```
