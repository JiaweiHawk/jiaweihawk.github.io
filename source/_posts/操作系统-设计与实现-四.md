...
#### 测试框架
  ...
  由于该实验对于正确性要求很高，因此按照前面的实验指南，首先搭建起一个测试框架——其使用本机环境代替QEMU环境，即将**AbstractMachine**提供的API使用glibc进行替换，然后进行测试即可。
  因此，首先创建测试目录**test**。为了兼容**kernel**目录下的API，我们添加部分条件编译，如下所示
  ```c
```
