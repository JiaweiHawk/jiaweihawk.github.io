...
static
#endif
void slab_init(uintptr_t offset) {
	...
	uintptr_t base_address = (((uintptr_t)buddy) + offset + sizeof(Slab_Per_Cpu) * cpu_counts);
...
#### 获取内存
  ...
  最后需要说明一下，这里仍然需要通过**buddy->ptr_page2idx**结构获取内存对象的大小——但是为了和前面的**Buddy**算法所分配的内存页进行区分，需要额外一个标志位。
  ```c
```
