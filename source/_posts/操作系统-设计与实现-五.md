...
```
  ...
  2. 即使解决了上面的问题(例如将程序的**stdout**重定向到**/dev/null**丢弃)，程序输出的字符串同样可能对解析产生麻烦，如下所示
  ```bash
...
### 解析COMMAND、ARG和环境变量
  ...
  对于ARG，实际上就是`argv[2:]`,其中**argv[0] == sperf**,**argv[1] == COMMAND**;另一方面，考虑到输出问题，我们还需要丢弃程序可能的输出，即需要额外添加两个参数**>**和**/dev/null"**;最后，由于我们还需要输出系统调用的时间信息，即我们还需要在额外添加**-T**参数
 ...
 * 对于参数，只需要添加" > /dev/null"，避免检测的程序输出影响解析即可
 */
char ** parse_args_environ(int argc, char *argv[]) {
	...
	exec_argv[argc] = REDIRECT_STDOUT_1;
	exec_argv[argc + 1] = REDIRECT_STDOUT_2;
