...
int syscall_info_sort_by_time_cmp(Syscall_Info **base, int i, int j) {
	...
	if(base[i]->time - base[j]->time < epsilon && base[j]->time - base[i]->time < epsilon) { return 0; }
	else if(base[i]->time - base[j]->time > epsilon) { return 1; }
	return -1;
}
...
int parse_strace_output(char *buf, int buf_size) {
	...
	double time = 0;
	regmatch_t pmatch[1];
	char *regex_syscall_so = buf, *regex_syscall_eo = buf, *regex_time_so = buf, *regex_time_eo = buf;
	...
	while(1) {
		...
		fprintf(stderr, "%.*s:%.*s\n", (int)(regex_syscall_eo - regex_syscall_so), regex_syscall_so, (int)(regex_time_eo - regex_time_so), regex_time_so);
		fflush(stderr);
		...
		//添加该系统调用及其消耗时间统计
		syscall_info_insert_and_sort(syscall, time);
