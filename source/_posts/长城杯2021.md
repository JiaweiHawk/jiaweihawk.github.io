...
### one_gadget和抬高相对栈基址的实际地址
  ...
  但如果添加一个条件，我们有指向`__malloc_hook`的指针，则问题就好办多了——因为`__realloc_hook`就在`__malloc_hook`旁边
...
```
  ...
  最后，则是普通的伪造chunk，从而修改**__malloc_hook**为**one_gadget**，这里展示一下抬栈姿势，我们就以`[rsp + 0x30] = 0`约束为例，其余是类似的。
