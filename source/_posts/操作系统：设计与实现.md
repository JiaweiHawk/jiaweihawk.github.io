...
#### 建树和打印
  ...
  这种形式的输出，可以使用**DFS**遍历，并且由于是一个树而非森林，则执行一遍**DFS**即可。当遍历到最后一个子进程时，直接一行打印即可，接着回退进入下一行输出即可。
  整个建树和打印的流程如下:
  1. 按照深度优先将处理当前进程入栈，并处理进程的下一个子进程，直到遇到没有子进程的进程为止。在这个过程中需要记录**最左侧下标**，以及**已成功处理的子进程数量**，为之后打印做准备
  2. 按照前面的记录信息，如果打印第一行，直接跳转到步骤3。否则如果已成功处理的子进程数量 + 1大于等于进程的子进程个数，则以空格填充；否则输出‘|’并填充"空格"对齐。
  3. 按照前面的记录信息，如果已成功处理的子进程个数 + 1大于等于进程的子进程个数，则打印**进程信息**。否则如果等于最左侧下标，输出‘+’并填充"-"对齐；否则输出'|'并填充空格对齐。这些通过"--"相连接。
