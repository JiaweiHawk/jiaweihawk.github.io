...
#### 系统进程关系
  ...
  由于这是一棵树，所以其很稀疏，因此我们就使用邻接数组的结构来存储这张树即可，其邻接数组中元素仍然是一个动态数组。

  这个部分仅仅涉及简单的文件读取，其基本步骤如下所示：
  1. 遍历前面获取的系统中的进程数组，根据进程**pid**，打开**/proc/[pid]/stat**文件
  2. 读取该文件的第4个字段值，即**ppid**字段，为进程的父进程pid。这里使用字符和状态机，跨过前三个字段信息。
    - 如果ppid为0，则该ppid不需要记录到关系中(因为默认根进程的pid为1)
    - 如果ppid非0，则该进程为ppid的子进程，则向ppid的邻接数组中添加即可该子进程即可

  这里看到频繁使用了动态数组这一结构，因此将其提取出来，使用**GCC**中相关的宏进行泛型编程，主要用到如下特性：
  1. `#`，GCC在预编译期间，将参数转换为字符串
  2. `##`，GCC在预编译期间，将两个符号连接在一起

  通过上述机制，既可以传递变量类型，从而完成泛型编程，具体代码如下所示
  ```c
